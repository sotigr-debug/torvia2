...
<div class="table-cell">
	...
	{elseif $field.Type == 'price'}
        ...
        {if $field.value.from}
            ...
            {str2money string=$field.value.from showCents=$listing_types[$item.Listing_type].Show_cents}
        {else}
            {$lang.any}
        {/if}
        ...
        {if $field.value.to}
            {$field.value.currency}
            {str2money string=$field.value.to showCents=$listing_types[$item.Listing_type].Show_cents}
        {else}
            {$lang.any}
        {/if}
