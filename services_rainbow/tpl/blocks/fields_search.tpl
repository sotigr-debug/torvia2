...
<div class="submit-cell">
    ...
    {elseif $field.Type == 'radio'}
        {if $field.Values && $field.Values|@count > 2}
          {assign var='cell_class' value='checkbox-field'}
        {else}
          {assign var='cell_class' value='couple-field'}
        {/if}
    ...
    <div class="field search-item {$cell_class}" id="sf_field_{$field.Key}">
        {if $field.Type == 'text'}
        ...
        {elseif $field.Type == 'number'}
            {if $aHooks.search_by_distance && $field.Key == $config.sbd_zip_field && is_file($sbd_file)}
                {include file=$sbd_file}
            {else}
                <input value="{if $fVal.$fKey.from}{$fVal.$fKey.from}{/if}" placeholder="{$lang.from}" class="numeric wauto" type="text" name="f[{$field.Key}][from]" maxlength="{if $field.Values}{$field.Values}{else}10{/if}" size="{if $field.Values}{$field.Values}{else}10{/if}" />
                <input value="{if $fVal.$fKey.to}{$fVal.$fKey.to}{/if}" placeholder="{$lang.to}" class="numeric wauto" type="text" name="f[{$field.Key}][to]" maxlength="{if $field.Values}{$field.Values}{else}10{/if}" size="{if $field.Values}{$field.Values}{else}10{/if}" />
            {/if}
        {elseif $field.Type == 'date'}
        ...
        {elseif $field.Type == 'mixed'}
            <input {if $fVal.$fKey.from}value="{$fVal.$fKey.from}"{/if} placeholder="{$lang.from}" class="numeric wauto" type="text" name="f[{$field.Key}][from]" size="9" maxlength="15" />
            <input {if $fVal.$fKey.to}value="{$fVal.$fKey.to}"{/if} placeholder="{$lang.to}" class="numeric wauto" type="text" name="f[{$field.Key}][to]" size="9" maxlength="15" />
            ...
            {elseif $df_source && $df_source|@count > 1}
