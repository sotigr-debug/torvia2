    ...
    <section class="side_block_search light-inputs">
        ...
        <div class="search-block-content{if is_array($search_forms) && $search_forms|@count == 1} no-tabs{/if}">
            {foreach from=$search_forms item='search_form' key='sf_key' name='sformsF'}
                ...
                <form method="{$listing_types[$search_form.listing_type].Submit_method}"
                    action="{strip}
                        {if $category.ID > 0}
                            {if $config.mod_rewrite}
                                {pageUrl key=$spage_key add_url="`$category.Path`/`$search_results_url`"}
                            {else}
                                {pageUrl key=$spage_key vars="category=`$category.ID`&`$search_results_url`"}
                            {/if}
                        {else}
                            {pageUrl key=$spage_key add_url=$search_results_url}
                        {/if}
                    {/strip}"
                >
                    ...
                    <div class="scroller{if $block.Side == 'top' || $block.Side == 'bottom'} row{/if}">
                        ...
                        <div class="search-button{if $block.Side == 'top' || $block.Side == 'bottom'} col-md-{if $side_bar_exists}4{else}3{/if} pt-0 ml-auto{/if}">
                            ...
                            {if !$in_category_search
                                && $listing_types[$search_form.listing_type].Advanced_search
                                && $listing_types[$search_form.listing_type].Advanced_search_availability
                            }
                                <a title="{$lang.advanced_search}" href="{pageUrl key=$spage_key add_url=$advanced_search_url}">
