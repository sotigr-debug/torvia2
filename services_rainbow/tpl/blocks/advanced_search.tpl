    ...
    <form class="advanced-search-form"
        ...
        action="{strip}
            {if $config.mod_rewrite}
                {pageUrl key=$pageInfo.Key add_url="`$advanced_search_url`/`$search_results_url`"}
            {else}
                {pageUrl key=$pageInfo.Key vars="`$search_results_url`&`$advanced_search_url`"}
            {/if}
        {/strip}"
    >
        ...
        <div class="submit-cell">
            ...
            <div class="field search-button">
                ...
                <a title="{$lang.quick_search}" href="{pageUrl key='search'}#{$listing_type.Key}">{$lang.quick_search}</a>
            </div>
        </div>
    </form>
</div>
