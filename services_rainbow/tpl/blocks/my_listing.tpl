            ...
            <ul>
                ...
                {else}
                    <li>
                        <div class="statuses">
                            {if $listing.Status == 'incomplete'}
                                <a href="{pageUrl key='add_listing' vars='incomplete='|cat:$listing.ID}"
                                    class="{$listing.Status}"
                                >
                                    {$lang[$listing.Status]}
                                </a>
                            {elseif $listing.Status == 'expired' || $listing.Status == 'approval'}
                                {if $pages.upgrade_listing}
                                    <a href="{pageUrl key='upgrade_listing' vars="id=`$listing.ID`"}"
                                        class="{$listing.Status}"
                                    >
                                {else}
                ...
                {if $pages.upgrade_listing
                    ...
                    && $available_plans
                    && $listing.Plan_type != 'account'
                }
                    <li>
                        <a title="{$lang.make_featured}"
                            class="nav_icon text_button"
                            href="{pageUrl key='upgrade_listing' add_url='featured' vars="id=`$listing.ID`"}"
                        >
                            {$lang.make_featured}
                        </a>
                    </li>
