<script type="text/javascript">
    ...
    rlPageInfo['url']        = '{pageUrl key=$pageInfo.Key}'; // @since 4.10.1
    ...
    rlConfig['host']                             = '{$domain_info.host}'; // @since 4.9.1
    ...
    rlConfig['static_files_revision']            = {$config.static_files_revision}; // @since 4.9.1
    rlConfig['static_map_provider']              = '{$config.static_map_provider}'; // @since 4.9.2
    rlConfig['gallery_max_zoom_level']           = '{if $config.gallery_max_zoom_level}{$config.gallery_max_zoom_level}{else}3{/if}'; // @since 4.9.3
    rlConfig['html_in_pages']                    = {if $config.html_in_pages}true{else}false{/if}; // @since 4.10.1
    rlConfig['html_in_categories']               = {if $config.html_in_categories}true{else}false{/if}; // @since 4.10.1
    rlConfig['user_country_code']                = {if $smarty.session.GEOLocationData}'{$smarty.session.GEOLocationData->Country_code}'{else}null{/if}; // @since 4.10.1
