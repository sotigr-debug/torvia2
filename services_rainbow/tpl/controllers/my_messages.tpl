        ...
        <div class="content-padding">
            <table class="list contacts-list">
                ...
                {foreach from=$contacts item='item' name='searchF' key='contact_id'}
                    ...
                    {assign var='urlVars' value="id=`$item.From`"}
                    {if $item.Admin}
                        {assign var='urlVars' value="`$urlVars`&administrator"}
                    {/if}
                    {if $item.Visitor_mail}
                        {assign var='urlVars' value="`$urlVars`&visitor_mail=`$item.Visitor_mail`"}
                    {/if}
                    ...
                    <tr class="body" id="item_{$contact_id|replace:'@':''|replace:'.':''}">
                        <td>
                            <label>
                                <input type="checkbox"
                                       name="del_mess"
                                       class="inline del_mess{if $item.Admin} admin{/if}"
                                       {if $item.From && $item.From != '-1'}id="contact_{$item.From}"{/if}
                                       {if $item.Visitor_mail}attr="{$item.Visitor_mail}"{/if}
                                />
                            </label>
                        </td>
                        <td valign="top">
                            <div class="picture{if !$item.Photo} no-picture{/if}">
                                <a href="{pageUrl key=$pageInfo.Key vars=$urlVars}"
                        ...
                        <td class="info">
                            ...
                            <a href="{pageUrl key=$pageInfo.Key vars=$urlVars}">
                                {$item.Message|html_entity_decode|strip_tags:false|nl2br|replace:'\n':'<br />'|truncate:120}
                            </a>
                        </td>
                    </tr>
                {/foreach}
