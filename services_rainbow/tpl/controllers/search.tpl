...
<ul class="tabs tabs-hash">
    {if is_array($search_forms) && $search_forms|@count > 0}
...
<div class="content-padding">
    {foreach from=$search_forms item='search_form' key='sf_key' name='sformsF'}
        ...
        <div id="area_{$sf_key|replace:'_':''}" class="tab_area{if !$smarty.foreach.sformsF.first || $keyword_search} hide{/if}">
            <form method="{$listing_types[$sf_key].Submit_method}"
                action="{pageUrl key=$spage_key add_url=$search_results_url}"
            >
                ...
                <div class="submit-cell">
                    ...
                    <div class="field search-button">
                        ...
                        {if $listing_types[$sf_key].Advanced_search
                            && $listing_types[$sf_key].Advanced_search_availability
                        }
                            <a title="{$lang.advanced_search}"
                                href="{pageUrl key=$spage_key add_url=$advanced_search_url}"
                            >
    ...
    <div id="area_keyword" class="tab_area{if !$keyword_search && is_array($search_forms) && $search_forms|@count > 0} hide{/if}">
        <form class="kws-block" method="post" action="{pageUrl key='search'}">
