...
{if $renew_id}
    ...
    <form id="form-checkout" name="payment" method="post" action="{pageUrl key=$pageInfo.Key vars="renew=`$renew_id`"}">
...
{elseif $purchase}
    ...
    {else}
        <form name="payment" method="post" action="{pageUrl key=$pageInfo.Key add_url='purchase'}">
...
{else}
    {if $packages}
        ...
        <a href="{pageUrl key=$pageInfo.Key add_url='purchase'}" class="button w-100 w-md-auto">
            {$lang.purchase_new_package}
        </a>
        ...
        {/literal}</script>
    {else}
        {pageUrl key=$pageInfo.Key add_url='purchase' assign='link'}
