...
{else}
    ...
    {if $featured}
        {assign var='addUrl' value='featured'}
    {/if}
    {if $smarty.get.id}
        {assign var='addVars' value="id=`$smarty.get.id`"}
    {else}
        {assign var='addVars' value="id=`$smarty.get.item`"}
    {/if}

    <div class="content-padding">
        <form method="post" action="{pageUrl key=$pageInfo.Key add_url=$addUrl vars=$addVars}">
            ...
            <div class="plans-container">
                ...
                <ul class="plans{if $plans && $plans|@count > 5} more-5{/if}{if $subscription_exists} with-subscription{/if}{if $featured_exists} with-featured{/if} scrollbar scrollbar_horizontal">
